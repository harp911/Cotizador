...
/* ────────────────────────────────────────────────────────────────
   COMBINED DESTINATION MODE
────────────────────────────────────────────────────────────────── */
function toggleCombinedMode() {
  state.isCombined = document.getElementById('chk-combined').checked;

  const combinedFields  = document.getElementById('combined-fields');
  const normalSection   = document.getElementById('normal-hotels-section');
  const dest1Section    = document.getElementById('hotels-dest1-section');
  const dest2Section    = document.getElementById('hotels-dest2-section');
  const globalPlanGroup = document.getElementById('global-plan-group');
  const lblDest         = document.getElementById('lbl-destination');

  if (state.isCombined) {
    combinedFields.style.display  = 'block';
    normalSection.style.display   = 'none';
    dest1Section.style.display    = 'block';
    dest2Section.style.display    = 'block';
    globalPlanGroup.style.display = 'none';
    if (lblDest) lblDest.innerHTML = 'Destino 1 <span class="req">*</span>';
    // Auto-add one hotel for each group if empty
    if (!state.hotels.some(h => h.group === 1)) addHotel(1);
    if (!state.hotels.some(h => h.group === 2)) addHotel(2);
  } else {
    combinedFields.style.display  = 'none';
    normalSection.style.display   = 'block';
    dest1Section.style.display    = 'none';
    dest2Section.style.display    = 'none';
    globalPlanGroup.style.display = '';
    if (lblDest) lblDest.innerHTML = 'Destino <span class="req">*</span>';
  }
}

function updateCombinedDuration() {
  const n1 = parseInt(document.getElementById('nights-dest1')?.value) || 0;
  const n2 = parseInt(document.getElementById('nights-dest2')?.value) || 0;
  const totalNights = n1 + n2;
  const totalDays   = totalNights + 1;
  const badge = document.getElementById('duration-badge');
  const text  = document.getElementById('duration-text');
  if (badge && text && totalNights > 0) {
    const d1name = document.getElementById('destination')?.value || 'Dest.1';
    const d2name = document.getElementById('destination-2')?.value || 'Dest.2';
    text.textContent = `${totalDays} dias / ${totalNights} noches (${n1}n ${d1name} + ${n2}n ${d2name})`;
    badge.style.display = 'flex';
  }
}

function recalcAllDest2Hotels() {
  state.hotels.filter(h => h.group === 2).forEach(h => recalcHotel(h.id));
}

function recalcAllHotels() {
  state.hotels.forEach(h => recalcHotel(h.id));
}
...
function recalcHotel(idx) {
  ...
  // In combined mode, Dest2 hotels include Dest1 cost in their PVF
  let costoTotal = costo;
  let costoNiñoTotal = costoNiño;
  let isCombinedDest2 = false;

  if (state.isCombined && hotel && hotel.group === 2) {
    const dest1Hotel = state.hotels.find(h => h.group === 1);
    if (dest1Hotel) {
      costoTotal     = costo + (dest1Hotel.costoBase || 0);
      costoNiñoTotal = costoNiño + (dest1Hotel.costoNiño || 0);
      isCombinedDest2 = true;
    }
  }

  const pvfStd      = calculatePrice(costoTotal, marginRec);
  const pvfFinal    = pvfStd ? roundToNearest1000(pvfStd * (1 - discountRec/100)) : null;
  const pvfNiñoStd  = calculatePrice(costoNiñoTotal, marginRec);
  const pvfNiñoFinal = pvfNiñoStd ? roundToNearest1000(pvfNiñoStd * (1 - discountRec/100)) : null;

  let html = '';
  const combinedLabel = isCombinedDest2 ? ' <span style="font-size:0.65rem;color:var(--teal-400);font-weight:600;">(combinado)</span>' : '';
  ...
  if (discountRec > 0) {
    html = `
      ...
      <div>Adulto: <strong style="color:var(--success); font-size:1.2rem;">${formatCOP(pvfFinal)}</strong> <span style="background:var(--success); color:#fff; font-size:0.65rem; padding:1px 4px; border-radius:4px; vertical-align:middle;">-${discountRec}%</span>${combinedLabel}</div>
    `;
    if (pvfNiñoFinal > 0) {
      html += `<div style="margin-top:2px; font-size:0.8rem; color:var(--teal-400)">Niño: <strong>${formatCOP(pvfNiñoFinal)}</strong></div>`;
    }
  } else {
    html = `<div>Adulto: <strong>${formatCOP(pvfStd)}</strong>${combinedLabel}</div>`;
    if (pvfNiñoStd > 0) {
  ...
  // If Dest1 hotel changed, recalculate all Dest2 prices
  if (state.isCombined && hotel && hotel.group === 1) {
    recalcAllDest2Hotels();
  }
}
...
function addHotel(group) {
  // In combined mode, limit Dest1 to 1 hotel
  if (group === 1 && state.hotels.filter(h => h.group === 1).length >= 1) return;
  ...
  let listId = 'hotels-list';
  if (group === 1) listId = 'hotels-list-dest1';
  if (group === 2) listId = 'hotels-list-dest2';
  ...
  // Label
  let cardLabel = `Hotel #${idx + 1}`;
  if (group === 1) cardLabel = '🏢 Hotel Destino 1 (Fijo)';
  if (group === 2) cardLabel = `🏝️ Opción #${state.hotels.filter(h => h.group === 2).length}`;

  card.innerHTML = `
    <div class="hotel-card-header">
      <span class="hotel-card-num">${cardLabel}</span>
...
function removeHotel(idx) {
  ...
  // Fallback: ensure at least one hotel in normal mode
  if (!state.isCombined && state.hotels.length === 0) addHotel();
}
...
function gatherFormData() {
  ...
  const diffDays = Math.round((end - start) / 86400000);
    ...
    .map(h => {
      ...
      if (state.isCombined && h.group === 2) {
        const dest1Hotel = state.hotels.find(x => x.group === 1);
        if (dest1Hotel) {
          const d1CostoEl = document.getElementById(`hotel-costo-${dest1Hotel.id}`);
          const d1Costo = d1CostoEl ? parseFloat(d1CostoEl.value.replace(/[^0-9.]/g, '')) || 0 : dest1Hotel.costoBase;
          const d1CostoNiñoEl = document.getElementById(`hotel-costonino-${dest1Hotel.id}`);
          const d1CostoNiño = d1CostoNiñoEl ? parseFloat(d1CostoNiñoEl.value.replace(/[^0-9.]/g, '')) || 0 : dest1Hotel.costoNiño;
          
          costoTotal = costo + d1Costo;
          costoNiñoTotal = costoNiño + d1CostoNiño;
        }
      }

      const pvfStd     = calculatePrice(costoTotal, margin);
      const pvfFinal   = pvfStd ? roundToNearest1000(pvfStd * (1 - discount / 100)) : null;
      const pvfNiñoStd = calculatePrice(costoNiñoTotal, margin);
  ...
  return {
    ...
    // Combined destination fields
    isCombined:   state.isCombined,
    destination2: document.getElementById('destination-2')?.value.trim() || '',
    nightsDest1:  parseInt(document.getElementById('nights-dest1')?.value) || 0,
    nightsDest2:  parseInt(document.getElementById('nights-dest2')?.value) || 0,
    planType1:    document.getElementById('plan-type-dest1')?.value || '',
    planType2:    document.getElementById('plan-type-dest2')?.value || '',
    fixedHotel:   state.isCombined ? (state.hotels.find(h => h.group === 1) || null) : null,
  };
}
...
function buildQuoteHTML(d) {
    ...
    return `
      <div class="q-hotel-card">
        ...
        <div class="q-hotel-info">
          ...
          <div class="q-hotel-plan">Plan: ${d.isCombined && d.destination2 ? `${d.planType1} (${d.destination}) + ${d.planType2} (${d.destination2})` : d.planType} — ${durationStr}</div>
          <div class="q-hotel-price-box">
            <div>
              <div class="q-hotel-price-label">Valor por persona</div>
              <div class="q-hotel-price-sub">
                ${d.isCombined && d.destination2 
                  ? `${d.days} días / ${d.nights} noches (${d.nightsDest1}n ${d.destination} + ${d.nightsDest2}n ${d.destination2})` 
                  : `${d.days} días / ${d.nights} noches — Plan: ${d.planType}`
                }
              </div>
            </div>
  ...
  let ticketsText;
  if (d.isCombined && d.destination2) {
    const origin = (d.flightLegs && d.flightLegs.length > 0) ? (d.flightLegs[0].origen_ciudad || 'Medellín') : 'Medellín';
    ticketsText = `Tiquetes aéreos ${origin} — ${d.destination} — ${d.destination2} — ${origin}`;
  } else {
    ticketsText = `Tiquetes aéreos Medellín — ${d.destination} — Medellín`;
      ...
      <div class="quote-body">
        ...
        <div class="quote-greeting">
          <p>
            ...
            ${d.isCombined && d.destination2
              ? `Te compartimos una propuesta combinada <span class="trip-highlight">${d.destination} + ${d.destination2}</span>${paxInfo}, con salida el <span class="trip-highlight">${d.dateStart}</span> y regreso el <span class="trip-highlight">${d.dateEnd}</span>.`
              : `Te compartimos una propuesta a <span class="trip-highlight">${d.destination}</span>${paxInfo}, con salida el <span class="trip-highlight">${d.dateStart}</span> y regreso el <span class="trip-highlight">${d.dateEnd}</span>.`
            }<br>
        ...
        <div class="q-conditions">
          <div class="q-box q-box-includes">
            ...
            <ul>
              <li>${ticketsText}</li>
              ${d.isCombined && d.destination2
                ? `<li>${d.nightsDest1} noches en <strong>${d.destination}</strong> &mdash; Plan <strong>${d.planType1}</strong>${d.fixedHotel ? ` (Hotel ${d.fixedHotel.name})` : ''}</li>
                   <li>${d.nightsDest2} noches en <strong>${d.destination2}</strong> &mdash; Plan <strong>${d.planType2}</strong></li>`
